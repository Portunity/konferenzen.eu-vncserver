...
ScreenCapture::ScreenCapture(HDC deviceToCapture, const POINT & cursorOffset) :
 _hScreenDC(deviceToCapture), _hCaptureDC(0), _hCaptureBitmap(), _backBuffer(0), _cursorOff(cursorOffset) {
	...
	//Der DC für das Zwischenspeichern muss kompatibel mit dem Desktop sein, sonst klappt das kopieren des Cursors nicht.
...
void ScreenCapture::capture(rfbScreenInfo* screen) {
	if (screen) {
		...
		//Cursor drauf zeichnen, damit der mit übertragen wird.
		POINT pt;
		GetCursorPos(&pt);
		...
		if (min_x <= max_x && min_y <= max_y) { //Änderung erkannt
